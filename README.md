...
当前在第四个里程碑基础上，完成 SubmitInt 与 future<int> 返回值支持。

- `ThreadPool.h`：类声明和成员，使用 pragma once 防止重复包含。
- `ThreadPool.cpp`：成员函数实现，保留原有 Submit 和排空队列行为；SubmitInt 使用 packaged_task 和 shared_ptr 包装任务，停止后抛出 runtime_error。
- `main.cpp`：11 个独立测试，每组输出 PASS/FAIL；失败返回非零退出码。
...
目前没有独立 Stop 接口，调用方须在析构开始前停止并等待所有提交线程；任务不得销毁自身线程池。未处理空任务、void Submit 任务异常、部分线程创建失败；这些已知边界不属于本次文件拆分成果。
本阶段验证（2026-09-16）：C++17 严格编译无警告，11/11 测试通过。新增覆盖单个 int 返回值、100 个 int 任务、void/int 混合提交、执行中及排队 int 任务的析构排空、停止池重复提交异常，以及任务异常经 future 传递后 worker 继续执行。停止路径用 0 和 -3 个线程构造的存活对象验证，不在析构后调用成员函数。

下一步按学习进度完善基础边界；尚未引入独立 Stop 接口或泛型 Submit。
...
第一版不加入 future、packaged_task、可变参数模板、动态扩缩容或无锁队列；当前阶段已在第一版基础上加入 int 返回值与 future、packaged_task。
...
成功时最后输出“测试结束：11/11 通过”，退出码为 0；worker 的创建日志允许交错。
