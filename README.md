...
当前已在 RAII 活动任务计数的基础上，完成统一提交接口：仅保留模板版 `Submit(F&& task, Arges&&... arges)`，支持 void、不同返回类型和带参数任务，统一返回对应 future；已补齐 `GetTaskCount()`、`GetActiveCount()`、`GetWorkerCount()`、`IsRunning()` 四个状态查询接口。本阶段完成 `Running / ShuttingDown / Stopped` 三态生命周期及显式 `Shutdown()`，支持幂等和多个外部线程并发关闭，析构统一调用 `Shutdown()`。当前项目使用 **C++17**。
...
- 模板版 `Submit` 使用 `std::lock_guard`，在同一锁内检查 `state == State::Running` 并入队，离开作用域释放锁后再通知 worker。
...
- `main.cpp`：31 个独立测试，每组输出 PASS/FAIL；失败返回非零退出码。
...
以上早期验证记录描述当时的接口行为；当前 API 以统一模板为准。停止状态提交统一抛出 `std::runtime_error("ThreadPool has stopped")`，不再返回 bool。早期测试用 0/-3 个线程构造的存活停止池验证拒绝行为；本阶段新增显式 Shutdown 后的拒绝测试，不在析构开始后调用 Submit。

下一步按学习进度继续完善空任务拒绝和部分线程创建失败时的回收。显式 Shutdown 已完成，动态扩缩容尚未实现。
...
实际学习已完成线程基础、任务队列与等待循环、固定 worker 和析构排空；已补线程数量和提交状态检查，阶段 5 的普通 Submit 任务异常隔离已完成，线程创建失败处理仍未完成；返回值扩展已完成模板版 Submit 和带参数任务提交，已加入任务统计与 RAII 活动计数保护，已完成统一 Submit 和四个状态查询接口，本阶段补齐三态生命周期与并发 Shutdown。下表保留最初学习路线，实际进度以上述里程碑为准。
...
任务统计与异常隔离已完成第八阶段，RAII 计数保护已完成第九阶段，统一提交与状态查询已完成第十阶段；后续继续补齐空任务拒绝与部分线程创建失败的回收，第十一阶段已完成显式 Shutdown，动态扩缩容仍未实现。

## 三态生命周期与显式关闭

正常生命周期为 `State::Running → State::ShuttingDown → State::Stopped`，不支持重新启动；无效线程数直接得到 Stopped 对象。
...
- `IsRunning() const`：与 Submit 和 Shutdown 使用同一把 `mtx`，在锁内判断 `state == State::Running`；true 表示仍接受任务，不代表当前有任务执行。关闭中、关闭完成或无效线程数构造均为 false。可在显式 Shutdown 后查询仍存活的对象，不能在析构开始后查询。
...
成功时最后输出“测试结束：31/31 通过”，退出码为 0；worker 的创建日志允许交错。
